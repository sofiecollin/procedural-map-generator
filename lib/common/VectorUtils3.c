...
// Row-wise by default but can be configured to column-wise (see SetTransposed)
	...
	mat4 R(GLfloat a, vec3 r)
	{
		...
		float c = (float) cos(a * 3.1415 / 180);
		float s = (float) sin(a * 3.1415 / 180);
		if (norm(r) != 1)
		{
			r = normalize(r);
		}
...
vec3::vec3(objl::Vector3 const& rhs)
	: x {rhs.X}, y {rhs.Y}, z{rhs.Z}
{
}

vec3& vec3::operator=(objl::Vector3 const& rhs)
{
	x = rhs.X;
	y = rhs.Y;
	z = rhs.Z;
