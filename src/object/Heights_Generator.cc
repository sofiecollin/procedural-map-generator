...
Heights_Generator::Heights_Generator(int size, int num_octaves, float smooth_factor, int seed)
    : size{size}, num_octaves{num_octaves}, smoothness{smooth_factor}, seed{seed}
{

}

float Heights_Generator::generate_height(int x, int z)
{
...
float Heights_Generator::generate_noise(int x, int z)
{
    srand(x+z*size+seed);    //sets the seed
    return ((float)rand() / (float)RAND_MAX);   //returns a random number between 0 and 1
}

float Heights_Generator::generate_smooth_noise(int x, int z)
{
...
float Heights_Generator::interpolate(float p1, float p2, float blend)
{
...
float Heights_Generator::generate_interpolated_noise(float x, float z)
{
...
void Heights_Generator::create_heightmap(int size)
{
